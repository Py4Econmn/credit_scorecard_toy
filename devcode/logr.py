...
def run_log():
    ...
    selected_dfs = []
    for dfi, indicator in zip(dfs, indicators):
        dfi['indicator'] = indicator
        selected_df = dfi[[f'{indicator}_bin' if not indicator == 'educ' else indicator, f'{indicator}_points','indicator']]
    ...
    # save score and data
    df[['income','age','educ','default','score']].to_csv('data\output_log.csv')
